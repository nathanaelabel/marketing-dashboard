...
document.addEventListener("DOMContentLoaded", function () {
    ...
    async function fetchAndUpdateChart(page = 1) {
        ...
        try {
            ...
            const normalizeData = () => {
                const visibleDatasets = transformedData.datasets.filter(
                    (dataset, index) =>
                ...
                transformedData.labels.forEach((label, branchIndex) => {
                    const visibleTotal = visibleDatasets.reduce(
                        (sum, dataset) => {
                            const value =
                                dataset.originalData[branchIndex]?.y || 0;
                    ...
                    if (visibleTotal > 0) {
                        visibleDatasets.forEach((dataset) => {
                            ...
                            const normalizedValue =
                                Math.max(0, originalValue) / visibleTotal;
                            dataset.data[branchIndex] = Math.max(
                                0,
                                normalizedValue
                    ...
                    plugins: {
                        ...
                        legend: {
                            ...
                            onClick: function (e, legendItem, legend) {
                                ...
                                const visibleDatasets =
                                    transformedData.datasets.filter(
                                        (dataset, idx) =>
                                            chart.isDatasetVisible(idx)
                                    );

                                transformedData.labels.forEach(
                                    (label, branchIndex) => {
                                        const visibleTotal =
                                        ...
                                        if (visibleTotal > 0) {
                                            transformedData.datasets.forEach(
                                                (dataset, idx) => {
                                                    if (
                        ...
                        datalabels: {
                            ...
                            display: function (context) {
                                ...
                                const categoryNames = chart.data.datasets.map(
                                    (d) => d.label
                                );
                                const mikaIdx = categoryNames.indexOf("MIKA");
                                const sparePartIdx =
                                    categoryNames.indexOf("SPARE PART");
                                const catIdx = categoryNames.indexOf("CAT");
                                const aksesorisIdx =
                                    categoryNames.indexOf("AKSESORIS");
                                const productImportIdx =
                                    categoryNames.indexOf("PRODUCT IMPORT");

                                const isVisible = (idx) =>
                                    idx !== -1 && chart.isDatasetVisible(idx);
                                ...
                                const isProductImportVis =
                                    isVisible(productImportIdx);
