...
class SalesMetricsController extends Controller
{
    public function getData(Request $request)
    {
        ...
        $stockValueQuery = "
            SELECT
              SUM(qtyonhand * plv.pricelist * 0.615) as stock_value
            FROM
              m_storage s
              INNER JOIN m_pricelist_version plv ON s.m_product_id = plv.m_product_id
              INNER JOIN m_locator loc ON s.m_locator_id = loc.m_locator_id
              INNER JOIN ad_org org ON s.ad_org_id = org.ad_org_id
            WHERE
              UPPER(plv.name) LIKE '%PURCHASE%' AND plv.isactive = 'Y'
              AND loc.value like ?
              AND org.name like ?
        ";
        $stockValueData = DB::selectOne($stockValueQuery, [$locationFilter, $locationFilter]);
        ...
        $arPieQuery = "
            SELECT 
                SUM(CASE WHEN h.duedate >= CURRENT_DATE - INTERVAL '30 days' THEN d.grandtotal ELSE 0 END) as days_1_30,
                SUM(CASE WHEN h.duedate BETWEEN CURRENT_DATE - INTERVAL '60 days' AND CURRENT_DATE - INTERVAL '31 days' THEN d.grandtotal ELSE 0 END) as days_31_60,
                SUM(CASE WHEN h.duedate BETWEEN CURRENT_DATE - INTERVAL '90 days' AND CURRENT_DATE - INTERVAL '61 days' THEN d.grandtotal ELSE 0 END) as days_61_90,
                SUM(CASE WHEN h.duedate < CURRENT_DATE - INTERVAL '90 days' THEN d.grandtotal ELSE 0 END) as days_90_plus
            FROM c_invoice h
            INNER JOIN c_invoice_v d ON h.c_invoice_id = d.c_invoice_id
            INNER JOIN ad_org org ON h.ad_org_id = org.ad_org_id
        ...
        return response()->json([
            ...
            'store_returns' => (float)($storeReturnsData->total_retur ?? 0),
