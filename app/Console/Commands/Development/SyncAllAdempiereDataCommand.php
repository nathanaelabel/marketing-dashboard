        ...
        if (!$targetConnection && !$skipStep1) {
            ...
            foreach ($singleSourceTables as $connection => $models) {
                ...
                foreach ($models as $modelName) {
                    ...
                    $params = ['model' => $modelName, '--connection' => $connection, '--type' => 'full'];
                    if (!$this->callWithRetries('app:sync-adempiere-table', $params, 3, null, 'app:sync-adempiere-table', $modelName)) {
        ...
        foreach ($connectionsToProcess as $connection) {
            ...
            if ($targetConnection && array_key_exists($connection, $singleSourceTables)) {
                ...
                foreach ($singleSourceTables[$connection] as $modelName) {
                    ...
                    $params = ['model' => $modelName, '--connection' => $connection, '--type' => 'full'];
                    if (!$this->callWithRetries('app:sync-adempiere-table', $params, 3, $connectionFailedSyncs, 'app:sync-adempiere-table', $modelName)) {
            ...
            foreach ($fullSyncTables as $tableInfo) {
                ...
                if (!$this->callWithRetries('app:sync-adempiere-table', $params, 3, $connectionFailedSyncs, 'app:sync-adempiere-table', $tableInfo['model'])) {
                    continue 2; // Skip to the next connection in the outer loop
                }
            }
            ...
            foreach ($fastSyncTables as $modelName) {
                ...
                if (!$this->callWithRetries('app:fast-sync-adempiere-table', $params, 3, $connectionFailedSyncs, 'app:fast-sync-adempiere-table', $modelName)) {
                    continue 2; // Skip to the next connection in the outer loop
                }
            }
