document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateBranchGrowthChart(dataFromServer) {
        ...
        if (branchGrowthChart) {
            ...
            branchGrowthChart.options.scales.y.ticks.callback = function (
                value
            ) {
                    ...
                    plugins: {
                        ...
                        datalabels: {
                            ...
                            formatter: function (value, context) {
                                ...
                                if (
                                    context.dataset.formattedData &&
                                    context.dataset.formattedData[
                                        context.dataIndex
                                    ]
                                ) {
                                    return context.dataset.formattedData[
                                        context.dataIndex
                                    ];
                                }
                                return value;
                            ...
                            callbacks: {
                                ...
                                afterLabel: function (context) {
                                    ...
                                    if (currentDatasetIndex > 0) {
                                        ...
                                        const previousValue =
                                            previousDataset.data[monthIndex];
    ...
    function showNoDataMessage(message) {
        ...
        const existingMessage =
            chartContainerElement.querySelector(".no-data-message");
        if (existingMessage) {
    ...
    function clearMessages() {
        const chartContainerElement = chartContainer.parentElement;
        const errorMessage =
            chartContainerElement.querySelector(".error-message");
        const noDataMessage =
            chartContainerElement.querySelector(".no-data-message");
    ...
    function fetchAndUpdateGrowthChart(
        branch,
        startYear,
        endYear,
        category,
        ...
        const url = `/branch-growth/data?branch=${encodeURIComponent(
            branch
        )}&start_year=${startYear}&end_year=${endYear}&category=${encodeURIComponent(
            category
        )}&type=${encodeURIComponent(type)}`;
        const filterSelectors = [
            "growth-start-year-select",
            "growth-end-year-select",
            "growth-branch-select",
            "growth-category-select",
            "growth-type-select",
        ];
    ...
    if (exportBtn) {
        exportBtn.addEventListener("click", function (e) {
            ...
            const exportUrl = `/branch-growth/export-excel?start_year=${currentStartYear}&end_year=${currentEndYear}&branch=${encodeURIComponent(
                currentBranch
    ...
    if (exportPdfBtn) {
        exportPdfBtn.addEventListener("click", function (e) {
            ...
            // Create download URL with parameters
            const exportPdfUrl = `/branch-growth/export-pdf?start_year=${currentStartYear}&end_year=${currentEndYear}&branch=${encodeURIComponent(
                currentBranch
            )}&category=${encodeURIComponent(
                currentCategory
            )}&type=${encodeURIComponent(currentType)}`;
