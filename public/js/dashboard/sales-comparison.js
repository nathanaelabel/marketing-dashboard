...
function updateDateHeaders(dateString) {
    ...
    const headers = ['sales-date-header', 'stok-date-header', 'bdp-date-header', 'stok-bdp-date-header', 'total-date-header'];
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const salesComparisonTable = new TableHelper({
        ...
        getAdditionalFilters: function () {
            ...
            return {
                date: dateSelect ? dateSelect.value : new Date().toISOString().split('T')[0]
            };
        },

        renderTable: function (data) {
            if (!data.data || data.data.length === 0) {
                this.elements.tableBody.innerHTML = '<tr><td colspan="17" class="px-3 py-4 text-center text-gray-500">No data available</td></tr>';
                return;
            }
            ...
            const rows = data.data.map(item => {
                ...
                return `
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.sales_mika)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.sales_sparepart)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.total_sales)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.stok_mika)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.stok_sparepart)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.total_stok)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.bdp_mika)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.bdp_sparepart)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.total_bdp)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.stok_bdp_mika)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.stok_bdp_sparepart)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.total_stok_bdp)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.total_mika)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 border-r border-gray-200 text-right">${formatCurrency(item.total_sparepart)}</td>
                        <td class="px-3 py-2 text-sm text-gray-900 text-right">${formatCurrency(item.grand_total)}</td>
                    </tr>
                `;
            }).join('');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', function (e) {
            ...
            const currentDate = document.getElementById('sales-comp-date-select').value;
